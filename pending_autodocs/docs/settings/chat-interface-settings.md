...
doc_meta:
  ...
  keywords: [settings, chat, interface, ui, notifications, tui, legacy]
---
...
Select the chat UI engine.

### Overview

Controls which UI engine to use for the chat interface. The legacy engine is the current default, while the TUI engine is a new terminal-based interface.
...
The UI engine is determined in this order:

1. **CLI flags** (highest): `--tui` or `--legacy-ui` (hidden flags)
...
```bash
KIRO_CHAT_UI=tui kiro-cli chat
```

### Notes

- The TUI engine requires embedded assets to be available in the build
- If TUI is requested but assets are not available, falls back to legacy UI automatically