...
doc_meta:
  ...
  keywords: [chat, conversation, agent, model, interactive, headless, mcp, require-mcp-startup, log, logging, history, KIRO_LOG_NO_COLOR, tui, legacy-ui, KIRO_CHAT_UI]
  related: [slash-chat-save, slash-chat-load, slash-agent, exit-codes, chat-interface-settings]
---
...
| `--tui` | | flag | Use the new terminal UI (conflicts with --legacy-ui) |
| `--legacy-ui` | | flag | Use the legacy terminal UI (conflicts with --tui) |
...
**UI Engine Selection**: The chat UI engine is determined by (in order of precedence):
1. CLI flags: `--tui` or `--legacy-ui`
2. Environment variable: `KIRO_CHAT_UI=tui` or `KIRO_CHAT_UI=legacy`
3. Setting: `chat.ui`
4. Default: `legacy`

If TUI is requested but assets are not embedded in the build, falls back to legacy UI automatically.
