...
const RESOURCES_PREFIX = "resource:///org/gnome/shell/extensions/amazon-q-for-cli-gnome-integration";
/**
 * @template {string[]} T
 * @param  {T} resources
...
export default class QCliExtension extends Extension {
    ...
    enable() {
        ...
        this.#resources = Gio.Resource.load(`${this.metadata.path}/resources/amazon-q-for-cli-gnome-integration.gresource`);
    ...
    #connect() {
        ...
        this.#queue.push(new Queue.Item(() => this.#sleep(100)
    ...
    #connect_to_mutter() {
        ...
        this.#connect_to_object(global.display, "notify::focus-window", () => {
            debug(`mutter: notify::focus-window on ${global.display.focus_window.get_wm_class()}`);
            if (this.#window !== global.display.focus_window) {
                this.#disconnect_from_object(this.#window, null);
                this.#window = global.display.focus_window;
                this.#connect_to_object(this.#window, "size-changed", () => this.#send_window_data(null));
        ...
        this.#connect_to_object(global.display, "grab-op-begin", (_, __, grab_op) => {
            if (grab_op === Meta.GrabOp.MOVING ||
                ...
                const cursor_connection = this.#connect_to_object(cursor, "position-invalidated", () => this.#send_window_data(null));
        ...
        new Promise((resolve) => {
            const attempt = () => {
                if (finished)
                    return;
                attempts++;
                debug(`Connecting to socket at: "${socket_path}" (${ordinal(attempts)} try)...`);
                client.connect_async(address, cancel, (socket_client, result) => {
                    ...
                    try {
                        ...
                        log_msg(`Encountered an error while connecting to socket at "${socket_path}" (${ordinal(attempts)} try).` +
    ...
    #disconnect() {
        if (this.#disconnecting) {
            debug("#disconnect: Ignoring disconnect call since already disconnecting");
            return;
        }
    ...
    #send_window_data(overlay_pressed) {
        ...
        debug(`Sending data for rect inner ${inner_rect.x},${inner_rect.y},${inner_rect.width},${inner_rect.height}` +
...
class PanelIcon extends PanelMenu.Button {
    ...
    constructor({ connected }) {
        ...
        const [icon_connected, icon_disconnected] = resource("icons/scalable/actions/q-connected.svg", "icons/scalable/actions/q-disconnected.svg");
