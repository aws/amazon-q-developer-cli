...
pub fn ser_model(
    ...
    if let Some(var_12) = &input.available_origins {
        let mut array_13 = object.key("availableOrigins").start_array();
    ...
    if let Some(var_15) = &input.additional_model_request_fields_schema {
        object.key("additionalModelRequestFieldsSchema").document(var_15);
    }
    Ok(())
}
