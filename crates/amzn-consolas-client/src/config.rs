...
impl Builder {
    ...
    /// impl amzn_consolas_client::config::auth::ResolveAuthScheme for CustomAuthSchemeResolver {
    ///     fn resolve_auth_scheme<'a>(
    ///         &'a self,
    ///         _params: &'a amzn_consolas_client::config::auth::Params,
    ///         _cfg: &'a ConfigBag,
    ///         _runtime_components: &'a RuntimeComponents,
    ...
    /// partition named `codewhisperer` (or `codewhisperer-<region>` if a region is configured).
    /// All CodeWhisperer clients without an explicit retry partition will share this default
    /// partition.
    ///
    /// # Notes
    ///
    ...
    /// ```no_run
    /// use amzn_consolas_client::config::Config;
    /// use amzn_consolas_client::config::retry::{
    ///     RetryPartition,
    ///     TokenBucket,
    /// };
    ///
    ...
    /// #    fn meta(&self) -> &amzn_consolas_client::error::ErrorMetadata { todo!() }
    /// # }
    ///
...
pub(crate) fn base_client_runtime_plugins(
    ...
    let scope = "amzn-consolas-client";
