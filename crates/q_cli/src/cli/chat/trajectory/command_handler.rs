...
impl<'a, W: Write> TrajectoryCommandHandler<'a, W> {
    ...
    async fn handle_checkpoint_command(
        &mut self,
        checkpoint_cmd: crate::cli::chat::command::CheckpointSubcommand,
    ) -> Result<(), ChatError> {
        ...
        match checkpoint_cmd {
            CheckpointSubcommand::Create { label } => {
                match self
                    ...
                    .lock()
                    .unwrap()
                    .create_checkpoint(&label, self.conversation_state)
                {
                    Ok(id) => {
                        execute!(
                            self.output,
            ...
            CheckpointSubcommand::List => match self.recorder.lock().unwrap().list_checkpoints() {
                Ok(checkpoints) => {
    ...
    fn handle_enable_command(&mut self) -> Result<(), ChatError> {
        ...
        let was_enabled = self.recorder.lock().unwrap().is_enabled();
        ...
        if !was_enabled {
            ...
            execute!(
                self.output,
                style::SetForegroundColor(Color::Green),
                style::Print("Trajectory recording is already enabled.\n"),
                style::SetForegroundColor(Color::Reset),
            )?;
        }
    ...
    fn handle_disable_command(&mut self) -> Result<(), ChatError> {
        ...
        if was_enabled {
            ...
            execute!(
                self.output,
                style::SetForegroundColor(Color::Yellow),
                style::Print("Trajectory recording is already disabled.\n"),
                style::SetForegroundColor(Color::Reset),
            )?;
        }
