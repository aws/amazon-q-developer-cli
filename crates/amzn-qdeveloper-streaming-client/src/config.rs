...
impl Builder {
    ...
    ///
    /// If `auth_scheme` has an existing
    /// [AuthSchemeId](aws_smithy_runtime_api::client::auth::AuthSchemeId) in the runtime, the
    /// current identity resolver and signer for that scheme will be replaced by those from
    /// `auth_scheme`.
    ///
    ...
    /// [The default auth scheme resolver](crate::config::auth::DefaultAuthSchemeResolver) will not
    /// recognize your custom auth scheme.
    ///
    /// # Examples
    /// ```no_run
    ...
    /// impl amzn_qdeveloper_streaming_client::config::auth::ResolveAuthScheme
    ///     for CustomAuthSchemeResolver
    /// {
    ///     fn resolve_auth_scheme<'a>(
    ///         &'a self,
    ///         _params: &'a amzn_qdeveloper_streaming_client::config::auth::Params,
    ///         _cfg: &'a ConfigBag,
    ///         _runtime_components: &'a RuntimeComponents,
    ...
    ///
    ///
    /// When unset, the client will used a generated endpoint resolver based on the endpoint
    /// resolution rules for `amzn_qdeveloper_streaming_client`.
    ///
    ///
    ...
    /// See the [`RetryPartition`](::aws_smithy_runtime::client::retries::RetryPartition)
    /// documentation for more details.
    ///
    /// # Default Behavior
    ///
    /// When no retry partition is explicitly set, the SDK automatically creates a default retry
    /// partition named `qdeveloperstreaming` (or `qdeveloperstreaming-<region>` if a region is
    /// configured). All QDeveloperStreaming clients without an explicit retry partition will
    /// share this default partition.
    ///
    /// # Notes
    ///
    /// - This is an advanced setting — most users won't need to modify it.
    /// - A configured client rate limiter has no effect unless
    ///   [`RetryConfig::adaptive`](::aws_smithy_types::retry::RetryConfig::adaptive) is used.
    ///
    /// # Examples
    ///
    ...
    /// ```no_run
    /// use amzn_qdeveloper_streaming_client::config::Config;
    /// use amzn_qdeveloper_streaming_client::config::retry::{
    ///     RetryPartition,
    ///     TokenBucket,
    /// };
    ///
    ...
    /// Like [`Self::retry_partition`], but takes a mutable reference to the builder and an optional
    /// `RetryPartition`
    ...
    /// Like [`Self::interceptor`], but takes a
    /// [`SharedInterceptor`](crate::config::SharedInterceptor).
    ...
    /// #    fn meta(&self) -> &amzn_qdeveloper_streaming_client::error::ErrorMetadata { todo!() }
    /// # }
    ///
    ...
    /// Like [`Self::retry_classifier`], but takes a
    /// [`SharedRetryClassifier`](::aws_smithy_runtime_api::client::retries::classifiers::SharedRetryClassifier).
    ...
    ///
    /// Note: this is used in combination with other endpoint rules, e.g. an API that applies a
    /// host-label prefix will be prefixed onto this URL. To fully override the endpoint
    /// resolver, use [`Builder::endpoint_resolver`].
...
impl ServiceRuntimePlugin {
    pub fn new(_service_config: crate::config::Config) -> Self {
        let config = {
            let mut cfg = ::aws_smithy_types::config_bag::Layer::new("AmazonQDeveloperStreamingService");
...
pub(crate) fn base_client_runtime_plugins(
    ...
    let scope = "amzn-qdeveloper-streaming-client";
