// Standalone program to index autodocs-v2 for introspect (V2 agent crate)
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if has_semantic && has_bm25 {
        println!("✓ autodocs-v2 already indexed (both semantic and BM25)");
        println!("\nExisting contexts:");
        for ctx in contexts {
    ...
    let autodocs_path = PathBuf::from("autodocs-v2/docs");
    if !autodocs_path.exists() {
        eprintln!("Error: autodocs-v2/docs not found");
    ...
    if !has_semantic {
        ...
        let request = AddContextRequest {
            ...
            description: "Kiro CLI V2 documentation (semantic)".to_string(),
            persistent: true,
    ...
    if !has_bm25 {
        ...
        let request = AddContextRequest {
            ...
            description: "Kiro CLI V2 documentation (BM25)".to_string(),
            persistent: true,
            include_patterns: Some(vec!["**/*.md".to_string()]),
            exclude_patterns: None,
    ...
    if has_semantic && has_bm25 {
        println!("\n✅ Success! V2 introspect will now use hybrid search (semantic + BM25)");
    } else {
