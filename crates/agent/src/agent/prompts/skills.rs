...
fn collect_skill_paths(resources: &[impl AsRef<str>], sys: &impl SystemProvider) -> Vec<(String, std::path::PathBuf)> {
...
mod tests {
    ...
    fn test_resolve_rejects_path_traversal() {
        assert!(!PROMPT_NAME_REGEX.is_match("../../etc/passwd"));
        assert!(!PROMPT_NAME_REGEX.is_match("path/name"));
        assert!(!PROMPT_NAME_REGEX.is_match("has space"));
        assert!(!PROMPT_NAME_REGEX.is_match(""));
    }
