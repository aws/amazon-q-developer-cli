...
/// When writing a match expression against `KiroSubscriptionType`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `kirosubscriptiontype` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `KiroSubscriptionType::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `KiroSubscriptionType::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `KiroSubscriptionType::NewFeature` is defined.
/// Specifically, when `kirosubscriptiontype` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `KiroSubscriptionType::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::str::FromStr for KiroSubscriptionType {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(KiroSubscriptionType::from(s))
    }
}
impl KiroSubscriptionType {
