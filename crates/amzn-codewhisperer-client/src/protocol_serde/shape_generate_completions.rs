    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "predictions" => {
                    builder = builder.set_predictions(crate::protocol_serde::shape_predictions::de_predictions(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
                ...
                "completions" => {
                    builder = builder.set_completions(crate::protocol_serde::shape_completions::de_completions(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
