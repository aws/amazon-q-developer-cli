...
async fn test_python_user_stories() -> Result<()> {
    ...
    if std::process::Command::new("pylsp").arg("--version").output().is_err() {
...
async fn test_multi_language_support() -> Result<()> {
    ...
    // Create Rust project
    let _rust_project = TestProject::create(ProjectConfig::rust_project(), &workspace_path)?;

    // Create TypeScript project
    let _ts_project = TestProject::create(ProjectConfig::typescript_project(), &workspace_path)?;
