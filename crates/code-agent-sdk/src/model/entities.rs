...
impl WorkspaceEditInfo {
    ...
    pub fn from_lsp_workspace_edit(edit: &lsp_types::WorkspaceEdit, workspace_root: &std::path::Path) -> Self {
...
/// Helper function to read multiple source lines from a file (from start_line to end_line
/// inclusive)
pub(crate) fn read_source_lines(file_path: &Path, start_line: u32, end_line: u32) -> Option<String> {
    let content = std::fs::read_to_string(file_path).ok()?;
    let lines: Vec<&str> = content.lines().collect();
    ...
    let selected_lines: Vec<String> = lines[start_idx..end_idx].iter().map(|s| s.to_string()).collect();
...
impl SymbolInfo {
    ...
    /// ```ignore
    pub fn from_workspace_symbol(symbol: &WorkspaceSymbol, workspace_root: &Path) -> Option<SymbolInfo> {
...
mod tests {
    ...
    fn test_read_source_lines() {
        ...
        assert_eq!(
            read_source_lines(&file_path, 2, 10),
            Some("line2\nline3\nline4".to_string())
        );
    }
