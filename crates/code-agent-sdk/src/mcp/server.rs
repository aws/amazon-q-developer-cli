    ...
    async fn ensure_client(&self, workspace_root: Option<PathBuf>) -> Result<(), ErrorData> {
        ...
        if client_guard.is_none() {
            ...
            let mut client = CodeIntelligence::builder()
                ...
                .build()
                .map_err(|e| {
                    ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Failed to create client: {e}"), None)
                })?;

            client
                .initialize()
                .await
                .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Failed to initialize: {e}"), None))?;
...
impl CodeIntelligenceServer {
    async fn detect_workspace_tool(&self, _arguments: Option<Map<String, Value>>) -> Result<CallToolResult, ErrorData> {
    ...
    async fn find_symbols_tool(&self, arguments: Option<Map<String, Value>>) -> Result<CallToolResult, ErrorData> {
        self.ensure_client(None).await?;
        ...
        let symbol_name = args
            .get("symbol_name")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ErrorData::new(ErrorCode::INVALID_PARAMS, "Missing symbol_name", None))?;

        let file_path = args.get("file_path").and_then(|v| v.as_str()).map(PathBuf::from);
        ...
        let symbols = client
            .find_symbols(request.clone())
            .await
            .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Find symbols failed: {e}"), None))?;
    ...
    async fn get_symbols_tool(&self, arguments: Option<Map<String, Value>>) -> Result<CallToolResult, ErrorData> {
        self.ensure_client(None).await?;
        ...
        let symbols = args
            ...
            .ok_or_else(|| ErrorData::new(ErrorCode::INVALID_PARAMS, "Missing symbols array", None))?
            .iter()
        ...
        let symbols = client
            .get_symbols(request)
            .await
            .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Get symbols failed: {e}"), None))?;
    ...
    async fn goto_definition_tool(&self, arguments: Option<Map<String, Value>>) -> Result<CallToolResult, ErrorData> {
        self.ensure_client(None).await?;
        ...
        let row = args
            .get("row")
            .and_then(|v| v.as_u64())
            .ok_or_else(|| ErrorData::new(ErrorCode::INVALID_PARAMS, "Missing row", None))? as u32;
        let column =
            args.get("column")
                .and_then(|v| v.as_u64())
                .ok_or_else(|| ErrorData::new(ErrorCode::INVALID_PARAMS, "Missing column", None))? as u32;
        let show_source = args.get("show_source").and_then(|v| v.as_bool()).unwrap_or(true);
        ...
        let definition = client
            ...
            .await
            .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Goto definition failed: {e}"), None))?;
    ...
    async fn rename_symbol_tool(&self, arguments: Option<Map<String, Value>>) -> Result<CallToolResult, ErrorData> {
        self.ensure_client(None).await?;
        ...
        let workspace_edit = client
            ...
            .await
            .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Rename symbol failed: {e}"), None))?;

        let response = if let Some(result) = workspace_edit {
            let message = if dry_run {
                format!(
                    "Dry-run: Would rename to '{}' ({} edits in {} files)",
                    new_name, result.edit_count, result.file_count
                )
            } else {
                format!(
                    "Renamed to '{}' ({} edits in {} files)",
                    new_name, result.edit_count, result.file_count
                )
            };
    ...
    async fn format_code_tool(&self, arguments: Option<Map<String, Value>>) -> Result<CallToolResult, ErrorData> {
        self.ensure_client(None).await?;
        ...
        let insert_spaces = args.get("insert_spaces").and_then(|v| v.as_bool()).unwrap_or(true);
        ...
        let edits = client
            .format_code(request)
            .await
            .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Format code failed: {e}"), None))?;

        let message = if edits > 0 {
            format!("Code formatted successfully ({edits} edits applied)")
        } else {
