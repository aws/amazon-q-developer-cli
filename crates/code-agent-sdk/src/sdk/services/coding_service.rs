...
pub trait CodingService: Send + Sync {
    ...
    async fn format_code(&self, workspace_manager: &mut WorkspaceManager, request: FormatCodeRequest) -> Result<usize>;
}
...
impl CodingService for LspCodingService {
    async fn rename_symbol(
        ...
        tracing::trace!(
            "Starting rename_symbol: file={:?}, row={}, col={}, new_name={}",
            request.file_path,
            request.row,
            request.column,
            request.new_name
        ...
        if !request.dry_run {
            if let Ok(Some(ref workspace_edit)) = result {
                ...
                if let Err(e) = apply_workspace_edit(workspace_edit) {
                    ...
                    // File changes are automatically detected by file watcher
                    tracing::trace!("Workspace edit applied, file watcher will handle LSP notifications");
                }
            }
        } else {
    ...
    async fn format_code(&self, workspace_manager: &mut WorkspaceManager, request: FormatCodeRequest) -> Result<usize> {
        ...
        if let Some(file_path) = &request.file_path {
            ...
            let edits = client.format_document(params).await?.unwrap_or_default();
