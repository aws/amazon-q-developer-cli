                ...
                let mut def_info = match response {
                    ...
                    GotoDefinitionResponse::Array(locations) => {
                        if let Some(location) = locations.first() {
                            DefinitionInfo::from_location(location, workspace_manager.workspace_root(), false)
                        } else {
                            return Ok(None);
                        }
                    },
                ...
                if show_source {
                    ...
                    for symbol in symbols {
                        let in_range = def_info.start_row >= symbol.start_row
                            ...
                            && (def_info.start_row != symbol.start_row || def_info.start_column >= symbol.start_column);
                        ...
                        if in_range {
                            ...
                            def_info.source_line = read_source_lines(&canonical_path, symbol.start_row, symbol.end_row);
                            break;
                        }
                    }
                }
...
mod tests {
    ...
    fn test_symbol_name_filtering_logic() {
        ...
        let exact_matches: Vec<_> = symbols.iter().filter(|s| s.name == "test_function").collect();
        assert_eq!(exact_matches.len(), 1);
        assert_eq!(exact_matches[0].name, "test_function");

        // Test contains match (should match "test_function", "my_test", "testing")
        let contains_matches: Vec<_> = symbols.iter().filter(|s| s.name.contains("test")).collect();
    ...
    fn test_document_symbol_to_symbol_info() {
        ...
        let result = LspSymbolService::document_symbol_to_symbol_info(&doc_symbol, &file_path, workspace_root);
