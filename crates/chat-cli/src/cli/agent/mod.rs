...
impl Agents {
    ...
    fn default_permission_label(&self, tool_name: &str) -> String {
        let label = match tool_name {
            ...
            name if ToolMetadata::GLOB.aliases.contains(&name) => "trust working directory".dark_green().bold(),
            name if ToolMetadata::GREP.aliases.contains(&name) => "trust working directory".dark_green().bold(),
