...
impl Agent {
    ...
    /// Retrieves an agent by name or path identifier. It does so via first seeking the given agent under local dir,
    ...
    pub async fn get_agent_by_name(os: &Os, agent_name: &str) -> eyre::Result<(Agent, PathBuf)> {
        // First try direct file path lookup (for path-based identifiers like "team/assistant")
        let config_path: Result<PathBuf, PathBuf> = 'config: {
            // Try path-based lookup first (local)
            let local_path_config = directories::chat_local_agent_dir(os)?.join(format!("{agent_name}.json"));
            if os.fs.exists(&local_path_config) {
                break 'config Ok(local_path_config);
            }

            // Try path-based lookup (global)
            let global_path_config = directories::chat_global_agent_path(os)?.join(format!("{agent_name}.json"));
            if os.fs.exists(&global_path_config) {
                break 'config Ok(global_path_config);
            }

            // Fallback to name-based lookup (local)
            ...
            // Fallback to name-based lookup (global)
        ...
        match config_path {
            ...
            _ => {
                // If direct path lookup failed, try loading all agents and find by name or path identifier
                let mut stderr = std::io::stderr();
...
impl Agents {
    ...
    pub fn switch(&mut self, name: &str, os: &Os) -> eyre::Result<&Agent> {
        ...
        let matching_key = self.agents.iter().find(|(_, agent)| {
    ...
    pub async fn load(
        ...
        let mut local_agents = 'local: {
            ...
            let results = load_agents_from_directory_recursive(&path, os, &mut global_mcp_config, mcp_enabled, output).await;
...
async fn load_agents_from_directory_recursive(
    dir_path: &Path,
    os: &Os,
...
mod tests {
    ...
    async fn test_switch_with_path_identifier() {
        ...
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().to_string(), "No agent with name nonexistent/path found");
    }
}
