...
use super::checkpoint::{CheckpointManager, truncate_message, CHECKPOINT_MESSAGE_MAX_LENGTH};
use super::cli::compact::CompactStrategy;
use super::cli::hooks::ToolContext;
use super::cli::model::{find_model, get_available_models, select_model};
use super::cli::SlashCommand;
use super::cli::editor::open_editor;
use super::cli::prompts::{GetPromptError, PromptsSubcommand};
use super::conversation::{ConversationState, TokenWarningLevel};
use super::input_source::InputSource;
use super::message::{AssistantMessage, AssistantToolUse, ToolUseResult, ToolUseResultBlock, UserMessage};
use super::parse::{ParseState, interpret_markdown};
use super::parser::{RecvErrorKind, RequestMetadata, SendMessageStream};
use super::prompt;
use super::token_counter::TokenCounter;
use super::tool_manager::{PromptQuery, PromptQueryResult, ToolManager, ToolManagerBuilder};
use super::tools::delegate::status_all_agents;
use super::tools::gh_issue::GhIssueContext;
use super::tools::{NATIVE_TOOLS, OutputKind, QueuedTool, Tool, ToolSpec};
use super::util::images::RichImageBlock;
use super::util::ui::draw_box;
use super::util::{animate_output, play_notification_bell, sanitize_unicode_tags};
...
use crate::cli::agent::{Agent, Agents, DEFAULT_AGENT_NAME, PermissionEvalResult};
use crate::cli::experiment::experiment_manager::{ExperimentManager, ExperimentName};
use crate::constants::{error_messages, tips, ui_text};
use crate::constants::ui_text::{
    LIMIT_REACHED_TEXT, POPULAR_SHORTCUTS, RESUME_TEXT,
    SMALL_SCREEN_POPULAR_SHORTCUTS, SMALL_SCREEN_WELCOME, WELCOME_TEXT,
};
use crate::database::settings::Setting;
use crate::os::Os;
use crate::telemetry::core::{
    AgentConfigInitArgs, ChatAddedMessageParams, ChatConversationType,
    MessageMetaTag, RecordUserTurnCompletionArgs, ToolUseEventBuilder,
};
...
pub fn trust_all_text() -> String {
    ui_text::trust_all_warning()
}

pub const TOOL_BULLET: &str = " ● ";
pub const CONTINUATION_LINE: &str = " ⋮ ";
pub const PURPOSE_ARROW: &str = " ↳ ";
pub const SUCCESS_TICK: &str = " ✓ ";
pub const ERROR_EXCLAMATION: &str = " ❗ ";
...
pub enum ChatError {
    ...
    Auth(#[from] AuthError),
    #[error("{0}")]
    SendMessage(Box<super::parser::SendMessageError>),
    #[error("{0}")]
    ResponseStream(Box<super::parser::RecvError>),
    #[error("{0}")]
...
impl From<super::parser::SendMessageError> for ChatError {
    fn from(value: super::parser::SendMessageError) -> Self {
        Self::SendMessage(Box::new(value))
    }
}

impl From<super::parser::RecvError> for ChatError {
    fn from(value: super::parser::RecvError) -> Self {
...
pub struct ChatSession {
    ...
    /// completion.
    pub tool_turn_start_time: Option<Instant>,
    ...
    pub pending_prompts: VecDeque<PromptMessage>,
    interactive: bool,
...
impl ChatSession {
        ...
        let (summary, request_metadata) = {
            loop {
                match response.recv().await {
                    Some(Ok(super::parser::ResponseEvent::EndStream {
                        message,
                        request_metadata,
                    })) => {
            ...
            match rx.recv().await {
                Some(Ok(msg_event)) => {
                    ...
                    match msg_event {
                        ...
                        super::parser::ResponseEvent::ToolUse(tool_use) => {
                            if self.spinner.is_some() {
                                drop(self.spinner.take());
                                queue!(
                                    self.stderr,
                        ...
                        super::parser::ResponseEvent::EndStream {
                            message,
                            request_metadata: rm,
                        } => {
    ...
    pub fn read_user_input(&mut self, prompt: &str, exit_on_single_ctrl_c: bool) -> Option<String> {
    ...
    pub fn reset_user_turn(&mut self) {
...
pub enum ActualSubscriptionStatus {
...
pub async fn get_subscription_status_with_spinner(
    os: &mut Os,
