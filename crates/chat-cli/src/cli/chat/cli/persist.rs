...
pub fn build_session_entries(
    ...
    conversations
        .into_iter()
        .map(|(conv_id, conv_state, _created_at, updated_at)| {
            let summary = conv_state.get_user_prompts().last().map_or_else(
                || "(empty conversation)".to_string(),
                |s| {
                    let single_line = s.replace(['\n', '\r'], " ");
                    if single_line.len() > 150 {
                        format!("{}...", &single_line[..150])
                    } else {
                        single_line
                    }
                },
...
pub fn list_conversations(os: &Os, writer: &mut impl std::io::Write) -> Result<(), ChatError> {
    ...
    execute!(
        ...
        StyledText::info_fg(),
        style::Print(format!(
            "\nChat sessions for {} (times in UTC{}):\n\n",
            cwd.display(),
            tz_offset
        )),
        StyledText::reset(),
