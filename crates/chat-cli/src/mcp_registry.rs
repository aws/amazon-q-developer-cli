...
pub fn resolve_registry_servers_for_agent_config(
    ...
    for (name, config) in agent_config.config().mcp_servers() {
        if let Some(reg) = config.registry_overrides() {
            ...
            missing_servers.insert(name.clone());
        }
    }
...
fn test_resolve_registry_servers_for_agent_config_npm() {
    ...
    // Agent config with tools referencing the registry server but no mcp_servers entry
    // (simulates what happens when "type": "registry" is dropped during deserialization)
...
fn test_resolve_registry_servers_skips_already_loaded() {
    ...
    // Agent config already has this server loaded as a Local (stdio) server
    ...
    // Should NOT overwrite the existing Local config with the registry Remote config
...
fn test_resolve_registry_servers_skips_not_in_registry() {
    ...
    // Server not in registry should not be added
    assert!(loaded.config().mcp_servers().is_empty());
}
