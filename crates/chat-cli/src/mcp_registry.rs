...
/// Convert V1's [`McpRegistryResponse`] into V2's structurally-identical counterpart.
///
/// V1 and V2 each carry their own copy of these registry types as a relic of the
/// V1→V2 split; the JSON shape is identical. This helper round-trips through serde
/// to bridge the two so V1 callers (e.g. the subagent) can construct V2's
/// [`chat_cli_v2::mcp_registry::RegistryAdapter`] and reuse the
/// [`agent::mcp::McpRegistry`] plumbing without duplicating its apply logic in V1.
///
/// An error here means the two type definitions have drifted — surface loudly
/// rather than silently dropping registry data.
pub fn to_v2_registry_response(
    response: &McpRegistryResponse,
) -> Result<chat_cli_v2::mcp_registry::McpRegistryResponse> {
...
#[test]
fn test_npm_registry_conversion_with_versioned_identifier() {
    use crate::cli::chat::tools::custom_tool::CustomToolConfig;
    ...
    assert_eq!(config.command, "npx");
    // Should NOT produce "@playwright/mcp@latest@1.0.0"
