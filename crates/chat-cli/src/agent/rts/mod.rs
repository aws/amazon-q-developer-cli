            ...
            &mut messages
                ...
                .map(|m| match m.role {
                    ...
                    Role::Assistant => {
                        ...
                        // the current model. Reasoning from a different model will be rejected.
                        let current_model_id = &self.model_id;
                        let is_auto = current_model_id.as_deref() == Some("Auto");
                        let reasoning_content = if is_auto {
                            // Auto means backend picks the model — can't guarantee continuity
                            None
                        } else {
