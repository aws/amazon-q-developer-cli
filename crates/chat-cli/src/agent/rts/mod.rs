        ...
        history.append(
            &mut messages
                ...
                .map(|m| match m.role {
                    ...
                    Role::Assistant => {
                        // Forward the last sealed thinking block for reasoning continuity.
                        // See `filter_reasoning_for_history` for the keep-last-sealed and
                        // model-continuity rules.
                        let reasoning_content = filter_reasoning_for_history(&m.content, &self.model_id);
...
/// Extract reasoning content to forward in conversation history, selecting the **last**
/// sealed thinking block produced this turn.
///
...
fn filter_reasoning_for_history(
    ...
    content.iter().rev().find_map(|c| {
        if let ContentBlock::Thinking(tb) = c {
            // Skip orphan thinking blocks: no signature AND no redacted content. Bedrock
            // rejects history containing such a block ("...thinking.signature: Field
            // required"). The live in-memory path never produces orphans (the agent loop
            // drops them at parse time), but keep-last selects the trailing block, which
            // is exactly where a truncated/orphan block would land — so guard here to
            // self-heal any orphan that reaches serialization from a persisted session.
...
mod tests {
    ...
    /// **last** one is forwarded into history — keep-last-sealed.
    #[test]
    fn test_filter_reasoning_keeps_last_of_multiple_sealed() {
        ...
        let content = vec![mk("first", "sig1"), mk("second", "sig2")];
        let current = Some("claude-opus-4.7".to_string());
        let reasoning = filter_reasoning_for_history(&content, &current).expect("a sealed block should be returned");
        assert_eq!(reasoning.text, "second", "should forward the last sealed block");
    ...
    /// is skipped and the preceding sealed block is forwarded — reverse-iteration
    /// self-heal that keep-last makes relevant.
    #[test]
    fn test_filter_reasoning_skips_trailing_orphan() {
        ...
        let current = Some("claude-opus-4.7".to_string());
        let reasoning = filter_reasoning_for_history(&content, &current)
