...
pub(crate) fn de_followup_prompt_event_payload(
    ...
    let depth = 0u32;
    let result =
        crate::protocol_serde::shape_followup_prompt_event::de_followup_prompt_event(tokens, _value, depth + 1)?
            .ok_or_else(|| {
                        ...
                        match key.to_unescaped()?.as_ref() {
                            "followupPrompt" => {
                                builder = builder.set_followup_prompt(
                                    crate::protocol_serde::shape_followup_prompt::de_followup_prompt(
                                        tokens,
                                        _value,
                                        depth + 1,
                                    )?,
