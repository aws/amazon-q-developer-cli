...
#[cfg(test)]
mod tests {
    /// Build-time version injection regression guard.
    ///
    /// `env!("CARGO_PKG_VERSION")` flows outbound to KRS as the `appVersion`
    /// header. KRS gates "thinking" on `appVersion >= 2.4.0`, so if the
    /// `0.0.0-dev` placeholder from `Cargo.toml` ever leaks into a built
    /// binary, every dev session silently runs models without thinking.
    /// `crates/chat-cli-v2/build.rs::inject_kiro_version()` overrides this.
    ...
    /// On-disk source-of-truth: lockfile-guard.yml requires both files to
    /// stay at `0.0.0-dev`. We assert that here so a local diff that bumps
    /// only one of them fails before reaching CI.
    #[test]
    fn tui_package_json_version_matches_cargo_workspace_version() {
