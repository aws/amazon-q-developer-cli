...
impl RtsModel {
    pub fn new(client: ApiClient, state: Arc<RtsState>) -> Self {
        Self { client, state }
    }

    pub fn state(&self) -> &Arc<RtsState> {
        &self.state
    }
        ...
        history.append(
            &mut messages
                ...
                .map(|m| match m.role {
                    ...
                    Role::Assistant => {
                        // Model-aware filtering: only include reasoning if it was generated by
                        // the current model. See `filter_reasoning_for_history` for details.
                        let current_model_id = self.state.model_id();
                        let reasoning_content = Self::filter_reasoning_for_history(&m.content, &current_model_id);
...
impl Model for RtsModel {
    ...
    fn refresh_auth(&self) -> Pin<Box<dyn std::future::Future<Output = bool> + Send + '_>> {
        // Force-refresh and persist a new token to the store. The SDK's lazy identity
        // cache serves its buffered identity until shortly before its expiry, so the
        // retry observes the new token precisely in the target case (the old token is
        // past expiry, forcing the cache to re-resolve from the store). Returns whether
        // a refreshable token was found.
        Box::pin(async move { crate::auth::force_refresh_active_token().await })
    }
}

/// Serializable snapshot of RTS model state.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RtsStateSnapshot {
    pub conversation_id: String,
    // TODO - don't use ModelInfo from chat::cli. There's other references already using it though
...
///
/// This type is shared between `AcpSession` (the source of truth) and `RtsModel`
/// (which runs inside the Agent actor on a separate task). Interior mutability
/// allows both to access/modify state through `&self`, avoiding the need to pass
/// `Arc<Mutex<T>>` everywhere.
///
/// Deadlock safety: All methods acquire the lock, perform a simple read/write,
/// and immediately release. No method holds the lock while calling external code.
///
/// ## Context
///
/// The RTS API requires passing the model id for the request, but this is dynamic
/// over the course of a conversation. Hence, instead of spawning some separate actor
/// to ensure a single source of truth, an interior mutability pattern using a `Mutex`
...
impl RtsState {
    pub fn new(conversation_id: String) -> Self {
        Self {
            inner: std::sync::Mutex::new(RtsStateSnapshot {
                conversation_id,
                model_info: None,
    ...
    pub fn conversation_id(&self) -> String {
        self.inner.lock().unwrap().conversation_id.clone()
    }
    ...
    pub fn context_usage_percentage(&self) -> Option<f32> {
        self.inner.lock().unwrap().context_usage_percentage
    }
    ...
    pub fn snapshot(&self) -> RtsStateSnapshot {
        self.inner.lock().unwrap().clone()
    }
}

impl std::fmt::Debug for RtsState {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RtsModelState")
    ...
    async fn fill_streamevent_buf(&mut self) -> Result<(), RecvError> {
        ...
        if let Some(ChatResponseStream::AssistantResponseEvent { content }) = peek {
            ...
            match self.peek().await? {
                Some(ChatResponseStream::CodeReferenceEvent {}) => (),
                _ => {
...
mod tests {
    ...
    /// rather than dying as a terminal Other.
    #[test]
    fn mid_stream_internal_server_error_is_transient_server_error() {
        ...
        let ise = amzn_codewhisperer_streaming_client::types::error::InternalServerError::builder()
            ...
            .build()
            .unwrap();
        let source = ApiClientError::CodewhispererChatResponseStream(SdkError::service_error(
            CodewhispererChatResponseStreamError::InternalServerError(ise),
    ...
    /// A mid-stream Unhandled error stays terminal: it classifies as Other with
    /// no status and no network signature, so transient_class yields None.
    #[test]
    fn mid_stream_unhandled_error_stays_terminal() {
        use amzn_codewhisperer_streaming_client::types::error::ChatResponseStreamError as CodewhispererChatResponseStreamError;
        use aws_smithy_types::event_stream::{
            Message,
            RawMessage,
        ...
        let err = recv_error_to_stream_error_kind(RecvError::Other { source });
        assert!(
            matches!(err.kind, StreamErrorKind::Other { .. }),
            "expected Other, got: {:?}",
            err.kind
        );
        assert_eq!(err.transient_class(), None);
    }
    ...
    async fn integ_test_rts_cancel() {
        ...
        let state = Arc::new(RtsState::new(Uuid::new_v4().to_string()));
    ...
    #[test]
    fn test_filter_reasoning_same_model_includes() {
        ...
        assert_eq!(result.unwrap().text, "reasoning");
    }

    #[test]
    fn test_filter_reasoning_different_model_strips() {
        use agent::agent_loop::types::ThinkingBlock;
        let content = vec![ContentBlock::Thinking(ThinkingBlock {
            text: "reasoning".into(),
            signature: Some("sig".into()),
        ...
        let current = Some("claude-sonnet-4.6".to_string());
        let result = RtsModel::filter_reasoning_for_history(&content, &current);
    ...
    #[test]
    fn test_filter_reasoning_legacy_block_no_model_id_strips() {
        use agent::agent_loop::types::ThinkingBlock;
        let content = vec![ContentBlock::Thinking(ThinkingBlock {
            text: "reasoning".into(),
            signature: Some("sig".into()),
            redacted_content: vec![],
            model_id: None, // legacy session
        })];
        let current = Some("claude-opus-4.7".to_string());
        let result = RtsModel::filter_reasoning_for_history(&content, &current);
        assert!(result.is_none());
