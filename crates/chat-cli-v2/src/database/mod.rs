...
pub enum Table {
    ...
    #[allow(dead_code)]
    Conversations,
    /// The conversations_v2 table contains user chat conversations with multiple per path.
    #[allow(dead_code)]
    ConversationsV2,
...
impl Database {
    ...
    pub fn get_conversation_by_path(
        ...
        let mut stmt =
            conn.prepare("SELECT value FROM conversations_v2 WHERE key = ?1 ORDER BY updated_at DESC LIMIT 1")?;
    ...
    pub fn set_conversation_by_path(
        ...
        Ok(conn.execute(
            ...
            params![path, conversation_id, value, now],
        )?)
    }
    ...
    pub fn list_conversations_by_path(
        &self,
        path: impl AsRef<Path>,
    ) -> Result<Vec<(String, serde_json::Value, i64, i64)>, DatabaseError> {
        let path = match path.as_ref().to_str() {
            Some(path) => path,
    ...
    pub fn get_conversation_by_id(&self, conversation_id: &str) -> Result<Option<serde_json::Value>, DatabaseError> {
        let conn = self.pool.get()?;
...
mod tests {
    ...
    async fn test_multiple_conversations_per_path() {
        ...
        let conv1: serde_json::Value = serde_json::from_str(conv1_json).unwrap();
        let conv2: serde_json::Value = serde_json::from_str(conv2_json).unwrap();

        // Save first conversation
        db.set_conversation_by_path(&test_path, "conv-1", &conv1).unwrap();

        // Wait to ensure different timestamps (millisecond precision)
        std::thread::sleep(std::time::Duration::from_millis(10));

        // Save second conversation
        db.set_conversation_by_path(&test_path, "conv-2", &conv2).unwrap();
