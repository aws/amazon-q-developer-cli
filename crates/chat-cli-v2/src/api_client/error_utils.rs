...
pub fn classify_get_usage_limits_error(api_error: &ApiClientError) -> GetUsageLimitsErrorType {
    match api_error {
        ApiClientError::GetUsageLimitsError(sdk_err) => match sdk_err.as_service_error() {
            Some(GetUsageLimitsError::AccessDeniedError(access_denied)) => match access_denied.reason() {
                Some(AccessDeniedExceptionReason::FeatureNotSupported) => GetUsageLimitsErrorType::FeatureNotSupported,
