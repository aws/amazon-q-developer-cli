...
pub struct ConversationState {
    ...
    /// Transient: rebuilt from AdditionalFields overrides on each request, not persisted.
...
pub enum ToolResultContentBlock {
    ...
    /// Skipped for serde because AwsDocument in Smithy doesn't implement Serialize/Deserialize.
    /// It is OK since today we only use serde for testing.
    /// We could also use FigDocument wrapper instead if we really need serde
    #[serde(skip)]
    Json(AwsDocument),
...
impl ChatResponseStream {
    ...
    pub fn len(&self) -> usize {
        match self {
            ...
            ChatResponseStream::CodeReferenceEvent {} => 0,
            ChatResponseStream::FollowupPromptEvent {} => 0,
            ChatResponseStream::IntentsEvent {} => 0,
            ChatResponseStream::InvalidStateEvent { .. } => 0,
            ChatResponseStream::MessageMetadataEvent { .. } => 0,
            ChatResponseStream::SupplementaryWebLinksEvent {} => 0,
    ...
    pub fn is_skippable_metadata(&self) -> bool {
        matches!(
            ...
            Self::MetadataEvent { .. }
                ...
                | Self::CodeReferenceEvent {}
                | Self::FollowupPromptEvent {}
                | Self::IntentsEvent {}
                | Self::SupplementaryWebLinksEvent {}
                | Self::Unknown {}
        )
    }
}
