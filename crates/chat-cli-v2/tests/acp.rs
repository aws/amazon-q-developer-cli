...
/// Regression: a per-model effort default set *in-session* via `/effort` must be
/// re-applied when the user switches away and back to that model via `/model`.
///
/// Repro of the reported bug: `/effort low` on opus-4.7, `/model` to opus-4.6,
/// then `/model` back to opus-4.7 wrongly showed the hardcoded `xhigh` instead of
/// the saved `low`. `/effort` persists the default through the session manager
/// (disk + the actor's authoritative settings), but the `/model` switch-back read
/// a stale per-session settings clone that never saw the in-session write, so the
/// saved value was lost.
#[tokio::test]
#[timeout(30000)]
#[serial]
async fn model_switch_back_applies_in_session_effort_default() {
    let (mut harness, client, session_id, _) = AcpTestHarnessBuilder::new("model_switch_back_in_session_effort")
        .with_setting("chat.defaultModel", serde_json::json!("claude-opus-4.7"))
        .with_trust_all(true)
        .build_with_session()
        .await;

    // Set effort to "low" in-session via the /effort command. This persists
    // chat.modelDefaults["claude-opus-4.7"].output_config.effort = "low" through
    // the session manager. Note: NOT pre-seeded into the settings file.
    let result = client
        .execute_command(
            session_id.clone(),
            serde_json::json!({ "command": "effort", "args": { "value": "low" } }),
        ...
        .await
        .expect("effort command failed");
    assert!(result.success, "effort set should succeed: {}", result.message);

    // Switch away to another model, then back to opus-4.7 via /model. The
    // switch-back resets additional fields from the schema and re-applies the
    // per-model defaults.
    client
        .prompt_text(session_id.clone(), "/model claude-opus-4.6")
        .await
        .expect("switch away failed");
    client
        .prompt_text(session_id.clone(), "/model claude-opus-4.7")
        .await
        .expect("switch back failed");

    // Capture the next request to inspect the effective effort on opus-4.7.
    harness
        .push_mock_responses_from_file(&session_id.0, "tests/mock_responses/simple_text.jsonl")
        .await;
    client
    ...
    let requests = harness.get_captured_requests(&session_id.0).await;
    assert!(!requests.is_empty(), "should have captured at least one request");
    let additional_fields = requests
        .last()
        .unwrap()
        .additional_model_request_fields
        .as_ref()
    ...
    // Must be the in-session "low", NOT the hardcoded "xhigh" for opus-4.7.
    assert_eq!(
        additional_fields["output_config"]["effort"], "low",
        "effort set in-session via /effort must survive a /model switch away and back \
         (got the stale/hardcoded value instead)"
    );
}

/// Regression (PICKER path): same bug as
/// `model_switch_back_applies_in_session_effort_default`, but the model change is
/// driven through the `session/set_model` request (the model PICKER) instead of
/// the `/model` slash command. The picker resolves to the `SetModel` handler,
/// which calls `update_model_info`. Before the fix that handler read a stale
/// per-session settings clone (`self.os.database.settings`, captured at session
/// start), so an effort default saved in-session via `/effort` was lost on
/// switch-back.
///
/// FAILS before the fix (effort reverts to the hardcoded `xhigh` for opus-4.7);
/// PASSES after threading the authoritative settings snapshot
/// (`AcpAgent::authoritative_settings`) into the SetModel path.
#[tokio::test]
#[timeout(30000)]
#[serial]
async fn picker_model_switch_back_applies_in_session_effort_default() {
    let (mut harness, client, session_id, _) = AcpTestHarnessBuilder::new("picker_model_switch_back_in_session_effort")
        .with_setting("chat.defaultModel", serde_json::json!("claude-opus-4.7"))
        .with_trust_all(true)
        .build_with_session()
        .await;
    ...
    // Switch away then back to opus-4.7 via the PICKER (set_session_model ->
    // SetModel handler), NOT the /model slash command. The switch-back resets
    // additional fields from the schema and re-applies the per-model defaults.
    client
        .set_session_model(session_id.clone(), "claude-opus-4.6".to_string())
        .await
        .expect("picker switch away failed");
    client
        .set_session_model(session_id.clone(), "claude-opus-4.7".to_string())
        .await
        .expect("picker switch back failed");
    ...
    assert_eq!(
        additional_fields["output_config"]["effort"], "low",
        "effort set in-session via /effort must survive a PICKER (set_model) switch \
         away and back (got the stale/hardcoded value instead)"
    );
}
...
/// With `chat.disableAutoDefaultModel = true`, switching models via /model must
/// NOT persist chat.defaultModel and must NOT show the "(saved as default)" suffix.
#[tokio::test]
#[timeout(30000)]
#[serial]
async fn model_switch_does_not_persist_when_auto_default_disabled() {
    let (harness, client, session_id, _) = AcpTestHarnessBuilder::new("model_switch_no_persist_disabled")
        .with_setting("chat.disableAutoDefaultModel", true)
        .with_trust_all(true)
        .build_with_session()
        .await;
    ...
    // The success message must NOT claim the preference was saved
    let captured = client.captured().await;
    let has_saved_message = captured.session_updates.iter().any(|update| {
        if let SessionUpdate::AgentMessageChunk(chunk) = update {
            format!("{:?}", chunk).to_lowercase().contains("saved")
        } else {
            false
        }
    });
    assert!(
        !has_saved_message,
        "Model switch message must not indicate persistence when auto-default is disabled"
    );

    // chat.defaultModel must be absent from the settings file
    let settings_content = std::fs::read_to_string(&harness.paths.settings_path).expect("failed to read settings file");
    let settings: serde_json::Value = serde_json::from_str(&settings_content).expect("failed to parse settings");
    assert!(
        settings.get("chat.defaultModel").is_none(),
        "chat.defaultModel must not be persisted when chat.disableAutoDefaultModel is true, got: {settings}"
    );
}
...
/// With `chat.disableAutoDefaultEffort = true`, /effort must NOT persist
/// chat.modelDefaults and must NOT show the "(saved for ...)" suffix.
#[tokio::test]
#[timeout(30000)]
#[serial]
async fn effort_does_not_persist_when_auto_default_disabled() {
    let (harness, client, session_id, _) = AcpTestHarnessBuilder::new("effort_no_persist_disabled")
        .with_setting("chat.disableAutoDefaultEffort", true)
        .with_trust_all(true)
        .build_with_session()
        .await;

    // Set effort to "low" via execute_command — should succeed but not persist
    let result = client
        .execute_command(
            session_id.clone(),
            serde_json::json!({ "command": "effort", "args": { "value": "low" } }),
    ...
    assert!(
        result.success,
        "effort execute should still succeed: {}",
        result.message
    );
    assert!(
        !result.message.to_lowercase().contains("saved"),
        "effort message must not indicate persistence when auto-default is disabled: {}",
        result.message
    );

    // chat.modelDefaults must be absent from the settings file
    let settings_content = std::fs::read_to_string(&harness.paths.settings_path).expect("failed to read settings file");
    let settings: serde_json::Value = serde_json::from_str(&settings_content).expect("failed to parse settings");
    assert!(
        settings.get("chat.modelDefaults").is_none(),
        "chat.modelDefaults must not be persisted when chat.disableAutoDefaultEffort is true, got: {settings}"
    );
}
