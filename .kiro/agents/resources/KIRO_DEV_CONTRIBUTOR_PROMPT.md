...
- **PR CI path**: check push access with `gh api repos/kiro-team/kiro-cli --jq .permissions` — contributors without `push` (even kiro-team org members) PR from a fork, where Rust CI runs automatically but TUI CI runs only while the `safe-to-test` label is present. The label is auto-removed after each run, so it must be re-applied after every push. Never apply `safe-to-test` yourself: it injects repo credentials into a workflow running the PR's code, and the gate exists so a human reviews the diff first — surface the need and let the user apply it. Merging is core-team-only: green + approved means hand it back, never attempt to merge
