...
| Push to kiro-cli | `gh api repos/kiro-team/kiro-cli --jq .permissions.push` | Branch PRs with full CI vs fork PRs (Rust CI automatic; TUI CI runs while the `safe-to-test` label is present — auto-removed after each run, re-apply after each push; `triage` permission suffices to apply it). Merge is core-team-only regardless |
