...
doc_meta:
  ...
  keywords: [login, auth, authentication, builder-id, identity-center, govcloud, device-flow, remote, headless]
  related: [logout, whoami, api-key-authentication]
---
...
The login command authenticates with Kiro CLI service. Supports Builder ID (free) and Identity Center (pro), including GovCloud regions. Local environments use a unified auth portal where users select their authentication method interactively. Remote environments support device flow for SSH/terminal-only access.
...
| Option | Description |
|--------|-------------|
| `--license <TYPE>` | License type: `pro` (Identity Center) or `free` (Builder ID) |
...
1. Opens browser
2. Select authentication method (Builder ID, Identity Center, or social providers)
...
**Local**: Opens browser with unified auth portal  
...
- [API Key Authentication](../features/api-key-authentication.md) - Alternative for headless/CI environments
...
**Alternative**: For headless/CI environments, use [API Key Authentication](../features/api-key-authentication.md) instead of interactive login.
