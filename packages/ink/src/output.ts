			...
			if (operation.type === 'write') {
				...
				if (clip) {
					...
					if (clipHorizontally) {
						lines = lines.map(line => {
							...
							const width = cachedStringWidth(line);
				...
				for (let [index, line] of lines.entries()) {
					...
					for (const character of characters) {
						// Clamp to output width to prevent overflow past terminal edge
						...
						const characterWidth = Math.max(1, cachedStringWidth(character.value));
