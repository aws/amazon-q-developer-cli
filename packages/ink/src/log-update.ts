...
// Creates a hybrid renderer that starts in standard (full-erase) mode and
// auto-promotes to incremental once the output has filled the first screen.
// This prevents ghost lines during startup when the layout is growing rapidly
// (standard mode does a clean eraseLines on every frame), while still getting
// the flicker-free incremental updates once the UI is stable.
const createHybrid = (
	stream: Writable & {rows?: number},
	{showCursor = false} = {},
): LogUpdate => {
	let promoted = false;
	let standard = createStandard(stream, {showCursor});
	let incremental: LogUpdate | undefined;

	const maybePromote = (str: string) => {
		if (promoted) return;
		const rows = stream.rows ?? 24;
		const lineCount = str.split('\n').length;
		if (lineCount >= rows) {
			promoted = true;
			// Transfer state: clear standard's tracked output, then sync incremental
			// to the same content so the handoff is seamless.
			incremental = createIncremental(stream, {showCursor});
			incremental.sync(str);
			// Also sync cursor state
			if (standard.isCursorDirty()) {
				// Cursor position is internal to standard, but we can't extract it.
				// The next setCursorPosition call will set it on incremental.
			}
		}
	};

	const active = (): LogUpdate => (promoted ? incremental! : standard);

	const render = (str: string) => {
		const result = active()(str);
		maybePromote(str);
		return result;
	};

	render.clear = () => active().clear();
	render.done = () => active().done();
	render.sync = (str: string) => {
		active().sync(str);
		maybePromote(str);
	};
	render.setCursorPosition = (position: CursorPosition | undefined) => {
		// Set on both so promotion doesn't lose cursor state
		standard.setCursorPosition(position);
		if (incremental) {
			incremental.setCursorPosition(position);
		}
	};
	render.isCursorDirty = () => active().isCursorDirty();
	render.willRender = (str: string) => active().willRender(str);
...
const create = (
	...
	if (incremental) {
		return createHybrid(stream as Writable & {rows?: number}, {showCursor});
	}
