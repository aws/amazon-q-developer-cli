...
describe('acquireSessionLock', () => {
  ...
  it('detects a lock held on a non-first physical copy', () => {
    ...
    expect(isSessionLocked('a')).toMatchObject({
      locked: true,
      pid: owner.pid,
    });
  });
  ...
  it('writes started_at (snake_case) and reads both key spellings', () => {
    ...
    writeFileSync(
      join(dir, '.lock'),
      JSON.stringify({ pid: owner.pid, started_at: stamp })
    );
    ...
    writeFileSync(
      join(dir, '.lock'),
      JSON.stringify({ pid: owner.pid, startedAt: legacyStamp })
    );
    expect(isSessionLocked('interop')).toMatchObject({
      locked: true,
      pid: owner.pid,
  ...
  it('checks only the selected physical store and skips remote rows', () => {
    ...
    ).toMatchObject({ locked: true, pid: owner.pid });
    expect(
