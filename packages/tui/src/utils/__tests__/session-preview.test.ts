...
describe('SessionPreviewProvider', () => {
  ...
  it('strips terminal escapes from preview text, titles, and tool names', () => {
    // The preview renders transcripts of arbitrary on-disk sessions on mere
    // row highlight, so OSC/DCS/bare-ESC sequences must not survive it.
    writeMeta(testDir, 'esc', { title: '\u001b]0;pwned\u0007legit title' });
    writeLog(testDir, 'esc', [
      {
        kind: 'Prompt',
        data: {
          message_id: 'm1',
          content: [
            { kind: 'text', data: '\u001b]52;c;ZXZpbA==\u0007steal clipboard' },
          ],
        },
      },
      {
        ...
        data: {
          message_id: 'm2',
          content: [
            { kind: 'text', data: '\u001bcreset' },
            { kind: 'tool_use', data: { name: '\u001b]0;x\u0007fs_read' } },
          ],
        },
      },
    ]);
    ...
    const provider = new SessionPreviewProvider(testDir);
    const preview = provider.getPreview('esc', 'v2');

    expect(preview!.summary.title).toBe('legit title');
    expect(preview!.summary.firstPrompt).toBe('steal clipboard');
    expect(preview!.summary.toolsSummary).toEqual(['fs_read']);
