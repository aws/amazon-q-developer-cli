...
describe('buildKasTurnTree', () => {
  ...
  it('strips terminal escapes from turn text and tool names', () => {
    // Transcript bytes are attacker-influenceable; OSC 52 (clipboard write)
    // and a bare ESC c (terminal reset) must never reach the renderer.
    writeMessages(dir, [
      { type: 'user', content: '\u001b]52;c;ZXZpbA==\u0007steal clipboard' },
      { type: 'turn_start', executionId: 'e1' },
      { type: 'assistant', executionId: 'e1', content: '\u001bcreset' },
      {
        type: 'tool_call',
        executionId: 'e1',
        toolName: '\u001b]0;pwned\u0007fs_read',
      },
    ]);

    const turns = buildKasTurnTree(dir);
    expect(turns[0]!.userText).toBe('steal clipboard');
    expect(turns[0]!.assistantText).toBe('reset');
