...
import {
  spawnLiveLockOwner,
  type LiveLockOwner,
} from '../../test-utils/live-lock-owner';
import {
...
let root: string;
let owner: LiveLockOwner;

beforeAll(() => {
  owner = spawnLiveLockOwner();
});
...
describe('deleteSession', () => {
  ...
  it('refuses a KAS session locked by another live process', () => {
    ...
    writeFileSync(
      join(dir, '.lock'),
      JSON.stringify({ pid: owner.pid, startedAt: new Date().toISOString() })
    );
...
describe('gcEmptySessions', () => {
  ...
  it('stops instead of deleting from disk when the agent refuses', async () => {
    // An answered refusal must not be overridden by the filesystem.
  ...
  it('falls back to disk when the agent cannot serve the delete', async () => {
    // A throw means "unanswerable" (old agent, transport error), which must
    // keep the documented store-level fallback rather than skipping the row.
    const dir = kasSession('kc', { old: true });
    const scan = await gcScan(null, new Set(), root);
  ...
  it('deletes from disk when no agent deleter is available', async () => {
    const dir = kasSession('kb', { old: true });
    const scan = await gcScan(null, new Set(), root);
