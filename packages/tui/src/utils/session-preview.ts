...
import { stripTerminalEscapes } from './sanitize-title.js';
import {
...
export class SessionPreviewProvider {
  ...
  private extractKasPreviewData(
    ...
    const title = stripTerminalEscapes((meta.title as string) || '');
 ...
 * Replaces newlines with spaces for single-line display, and strips terminal
 * escapes: transcript bytes are attacker-influenceable (tool output, pasted
 * content, model output) and the renderer only filters dangerous CSI, so OSC
 * (including clipboard writes), DCS/APC strings and bare ESC finals would
 * otherwise reach the terminal from a merely highlighted row.
 */
function truncate(text: string, maxLen: number): string {
  const singleLine = stripTerminalEscapes(text)
    .replace(/[\n\r]+/g, ' ')
    .trim();
