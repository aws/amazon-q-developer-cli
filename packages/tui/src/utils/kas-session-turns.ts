...
function parseSubagent(
  ...
  for (const e of events) {
    ...
    if (t === 'tool_call') toolCallCount++;
      ...
      const text = stripTerminalEscapes(payloadText(e.payload))
        .replace(/\s+/g, ' ')
        .trim();
 ...
 */
// LINT-DEBT(complexity): pre-existing at gate adoption; Function 'buildKasTurnTree' has a complexity of 39. Maximum allowed is 30.; refactor before extending
...
export function buildKasTurnTree(sessionDir: string): SessionTurn[] {
  ...
  for (const e of events) {
    const p = e.payload;
    if (!p) continue;
    const text = stripTerminalEscapes(payloadText(p))
      .replace(/\s+/g, ' ')
      .trim();
    ...
    switch (p.type) {
      ...
      case 'assistant':
        ...
        break;
      case 'tool_call': {
        const name = stripTerminalEscapes(p.toolName || p.name || '');
    ...
    if (e.kind === 'Prompt' && Array.isArray(e.data.content)) {
      ...
      for (const block of e.data.content) {
        ...
        if (b?.kind === 'tool_use' && b.data?.name) {
          current.toolNames.push(stripTerminalEscapes(b.data.name));
        }
      }
    }
  }
