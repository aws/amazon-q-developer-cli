...
async function readV2Store(cliDir: string): Promise<SessionListingResult> {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      // Names the file: the dashboard only reports that something was
      // skipped, so without this the cause cannot be traced from a log.
      logger.debug('[all-workspace-sessions] skipped v2 session', {
        sessionId,
        err,
      });
      complete = false;
...
async function readKasStore(
  ...
  for (const copies of bySession.values()) {
    ...
    if (!copy) {
      logger.debug('[all-workspace-sessions] no usable KAS copy', {
        sessionId: copies[0]?.sessionId,
      });
      complete = false;
      continue;
    }
    ...
    if (entry) entries.push(entry);
      ...
      logger.debug('[all-workspace-sessions] skipped KAS session', {
        sessionId: copy.sessionId,
      });
      complete = false;
    }
...
export async function listLiveDashboardSessionsDetailed(
  ...
  if (sessionLister.listAllWorkspaceSessions) {
    try {
      ...
      if (!all.failed) {
        // A partial listing drives the dashboard's incomplete warning, so
        // record it: this is otherwise the one path that degrades silently.
        if (all.complete === false) {
          logger.warn(
            '[all-workspace-sessions] engine reported a partial live listing',
      ...
      logger.warn(
        '[all-workspace-sessions] engine reported the live listing as failed',
        { sessions: all.sessions.length }
      );
    } catch (err) {
      logger.warn('[all-workspace-sessions] all-workspace listing threw:', err);
    }
  }
  try {
    const scoped = await sessionLister.listSessions(cwd);
    // The cwd-scoped fallback cannot see other workspaces, so the catalog is
    // incomplete by construction — say so rather than leaving it unexplained.
    logger.warn(
      '[all-workspace-sessions] fell back to the cwd-scoped live listing',
      { cwd, sessions: scoped.sessions.length }
    );
    return normalizeListingResult(scoped.sessions, false);
  } catch (err) {
    logger.warn('[all-workspace-sessions] live listing failed:', err);
    return { sessions: [], complete: false };
  }
}
