...
export class KasAcpClient extends BaseAcpClient {
   ...
   * deleted from the backend. Resolves false on refusal OR when the agent
   * predates the method (it is served unconditionally but not listed in
   * `extensionMethods`, so try-and-fallback IS the feature detection).
   */
  async deleteSessionById(
    ...
    try {
      ...
      logger.debug('[kas] session/delete failed:', e);
      return false;
    }
  }
