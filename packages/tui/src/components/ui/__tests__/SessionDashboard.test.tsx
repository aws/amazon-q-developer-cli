...
mock.module('../../../utils/session-mutations.js', () => ({
  ...
  deleteLocalKasSessionWithAgent: async (
    ...
    // Mirrors the real function: an agent that cannot serve the delete throws,
    // and the store-level path takes over.
    if (await agentDelete(id).catch(() => false)) {
      return { ok: true, store: 'kas' as const };
    }
    deletedIds.push(id);
    return deleteOutcome.ok
