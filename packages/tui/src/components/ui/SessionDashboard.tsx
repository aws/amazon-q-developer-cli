    ...
    async (staged: {
      ...
      if (isCloud) {
        if (
          ...
          return { ok: false, reason: 'cloud delete failed' };
        }
        // The RPC throws when the engine or agent cannot serve it; a cloud row
        // has no local store to fall back to, so report it as a failure.
        try {
          const deleted = await kiro.deleteSessionById(staged.sessionId, {
            source: 'remote',
          });
          return {
            ok: deleted,
          ...
          logger.warn('[dashboard] cloud delete failed:', err);
          return { ok: false, reason: 'cloud delete failed' };
        }
      }
