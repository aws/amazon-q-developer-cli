...
export const McpPanel: React.FC<McpPanelProps> = ({ servers, onClose }) => {
  ...
  const filtered = search
    ? servers.filter((s) => s.name.toLowerCase().includes(q) || s.status.toLowerCase().includes(q))
    : servers;
  ...
  const maxNameLen = servers.reduce((max, s) => Math.max(max, s.name.length), 0);
    ...
    >
      {servers.length === 0 ? (
        ...
        <Box flexDirection="column">
          ...
          {visible.map((server) => (
            <Box key={server.name}>
              <Box width={nameCol}><Text>{primary(server.name)}</Text></Box>
              <Box width={statusCol}><Text>{statusColor(server.status)(statusLabels[server.status])}</Text></Box>
              <Text>{dim(`${server.toolCount} tool${server.toolCount === 1 ? '' : 's'}`)}</Text>
            </Box>
          ))}
        </Box>
      )}
