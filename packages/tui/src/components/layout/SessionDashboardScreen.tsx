...
export const SessionDashboardScreen: React.FC = () => {
  ...
  const refreshSessions = useCallback(
    async (resetCache = false) => {
      ...
      const incomplete = !disk.complete || !live.complete || !classic.complete;
      setCatalogIncomplete(incomplete);
      // The footer only says "some sessions couldn't be loaded"; naming the
      // source is what makes a user report traceable.
      if (incomplete) {
        logger.warn('[dashboard] catalog incomplete', {
          disk: disk.complete,
          live: live.complete,
          classic: classic.complete,
          counts: {
            disk: disk.sessions.length,
            live: live.sessions.length,
            classic: classic.sessions.length,
          },
        });
      }
