...
export class Kiro {
  ...
  /**
   * Agent-routed session delete. False when the agent answered and refused;
   * throws when the engine or agent cannot serve the call at all, so callers
   * can keep a store-level fallback for that case only.
   */
  async deleteSessionById(
    sessionId: string,
    options?: { source?: 'local' | 'remote' }
  ): Promise<boolean> {
    if (!this.sessionClient?.deleteSessionById) {
      throw new Error('Session delete is not supported by this engine');
    }
    ...
    return this.runSessionTransition(async () => {
      ...
      try {
        ...
        try {
          ...
          // Stop an abandoned load from committing later: the client checks
          // this at every post-await juncture, so a late RPC response cannot
          // seize ownership after the rollback below.
          sessionClient.abandonPendingLoad?.();
