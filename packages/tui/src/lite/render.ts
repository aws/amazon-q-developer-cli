 ...
 * lite TUI. Previously split across render/{text,theme,markdown,tools,subagent,
 * message}.ts plus a barrel; collapsed into this single module (the split had
 * no consumers beyond intra-module imports + the test files, which already
 * import from `../render.js`). The companion diff renderer stays in `./diff.js`
 * (it imports a few helpers from here; the cycle is runtime-only).
 */
