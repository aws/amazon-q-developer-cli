...
/** Run `fn`, prefixing the step label onto any thrown error so the failing
 * step is named in the test output (the test runner captures the snapshot). */
...
describe.skipIf(!KR_ENABLED)(
  ...
  () => {
    ...
    it('survives shell streaming, mid-stream cancel, and a follow-up turn with append-only intact', async () => {
      ...
      await step('boot lite + wait for prompt', async () => {
      ...
      await step('turn 1: short hello prompt', async () => {
      ...
      await step('turn 2: shell tool streaming + complete', async () => {
        const cmd =
      ...
      await step(
        ...
        async () => {
          const s = await testCase!.getStore();
      ...
      await step('turn 3: 6-chunk stream + Ctrl+C mid-stream', async () => {
        for (let i = 0; i < 6; i++) {
          await testCase!.pushSendMessageResponse([
      ...
      await step(
        ...
        async () => {
          const s = await testCase!.getStore();
          const toolMessages = s.messages.filter((m) => m.role === 'tool_use');
          // Only the turn-2 shell tool should be in messages — turn 3 had no
          // tool, so total tool_use count is exactly 1.
      ...
      await step(
        'after Ctrl+C: liveOutputs has no leftover entries',
        async () => {
          // liveOutputs is keyed by tool-call id; turn 3 had no tool call and
          // the turn-2 shell tool id was already cleared above.
          const s = await testCase!.getStore();
      ...
      await step('turn 4: final clean prompt', async () => {
      ...
      await step('after turn 4: idle with all four turns visible', async () => {
        const s = await testCase!.getStore();
        expect(s.isProcessing).toBe(false);
