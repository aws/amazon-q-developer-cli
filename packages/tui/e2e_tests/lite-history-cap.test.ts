...
describe('lite history cap [bug-mine 2.7]', () => {
  ...
  // Skipped on win32: the fixture seeds 40 turns through a second chat_cli via
  // launchAcpHelper, whose IPC connect-back is windows-flaky on contended
  // runners (same reason chat-command.test.ts win32-skips its acp-helper
  // cases). ubuntu + macos run it and exercise the resume render cap fully.
  it.skipIf(process.platform === 'win32')(
    'caps rendered history at LITE_HISTORY_RENDER_CAP on session resume',
    async () => {
