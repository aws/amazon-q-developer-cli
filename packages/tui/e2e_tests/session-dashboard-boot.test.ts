  ...
  it('opens the dashboard without creating a chat session and exits on close', async () => {
    testCase = new E2ETestCase({
      ...
      extraEnv: {
        ...
        // The Windows harness spawns the TUI directly, bypassing the Rust
        // launcher's rollout export — pass the feature explicitly so the
        // boot gate sees it on every platform.
        KIRO_ENABLED_FEATURES: '["session_dashboard"]',
