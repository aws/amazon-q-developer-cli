...
function extractMarkerLines(snapshot: string[], marker: string): string[] {
  return snapshot.filter(line => line.includes(marker)).map(l => l.trimEnd());
}
...
describe('lite resize stability [bug-mine 1.7]', () => {
  ...
  it('old static rows stay byte-for-byte intact after terminal resize', async () => {
    ...
    await testCase.pushSendMessageResponse([
      { kind: 'event', data: { kind: 'AssistantResponseEvent', data: { content: 'TURN_1_MARKER_ALPHA' } } },
    ]);
    ...
    await testCase.waitForText('TURN_1_MARKER_ALPHA', 15000);
    await testCase.waitForIdle(10000);

    // --- Turn 2 ---
    await testCase.pushSendMessageResponse([
      { kind: 'event', data: { kind: 'AssistantResponseEvent', data: { content: 'TURN_2_MARKER_BRAVO' } } },
    ]);
    await testCase.pushSendMessageResponse(null);
    ...
    await testCase.sleepMs(100);
    await testCase.pressEnter();
    await testCase.waitForText('TURN_2_MARKER_BRAVO', 15000);
    await testCase.waitForIdle(10000);

    // --- Turn 3 ---
    await testCase.pushSendMessageResponse([
      { kind: 'event', data: { kind: 'AssistantResponseEvent', data: { content: 'TURN_3_MARKER_CHARLIE' } } },
    ]);
    await testCase.pushSendMessageResponse(null);
    ...
    await testCase.sleepMs(100);
    await testCase.pressEnter();
    await testCase.waitForText('TURN_3_MARKER_CHARLIE', 15000);
    await testCase.waitForIdle(10000);
    ...
    const marker3Before = extractMarkerLines(snapBefore, 'TURN_3_MARKER_CHARLIE');
    ...
    const marker1AfterWide = extractMarkerLines(snapAfterWide, 'TURN_1_MARKER_ALPHA');
    const marker2AfterWide = extractMarkerLines(snapAfterWide, 'TURN_2_MARKER_BRAVO');
    const marker3AfterWide = extractMarkerLines(snapAfterWide, 'TURN_3_MARKER_CHARLIE');
    ...
    // No duplicate markers (no re-emission of static items)
    expect(marker1AfterWide.length).toBe(1);
    expect(marker2AfterWide.length).toBe(1);
    expect(marker3AfterWide.length).toBe(1);
    ...
    await testCase.pushSendMessageResponse([
      { kind: 'event', data: { kind: 'AssistantResponseEvent', data: { content: 'POST_RESIZE_WIDE_DELTA' } } },
    ]);
    await testCase.pushSendMessageResponse(null);

    await testCase.sendKeys('wide');
    await testCase.sleepMs(100);
    await testCase.pressEnter();
    await testCase.waitForText('POST_RESIZE_WIDE_DELTA', 15000);
    await testCase.waitForIdle(10000);

    const snapAfterNewMsg = testCase.getSnapshot();
    expect(snapAfterNewMsg.some(l => l.includes('POST_RESIZE_WIDE_DELTA'))).toBe(true);

    // Old markers still intact after new message at new width
    expect(extractMarkerLines(snapAfterNewMsg, 'TURN_1_MARKER_ALPHA')[0]).toBe(marker1Before[0]);
    expect(extractMarkerLines(snapAfterNewMsg, 'TURN_2_MARKER_BRAVO')[0]).toBe(marker2Before[0]);
    expect(extractMarkerLines(snapAfterNewMsg, 'TURN_3_MARKER_CHARLIE')[0]).toBe(marker3Before[0]);
    ...
    const narrowMarker1 = extractMarkerLines(snapAfterNarrow, 'TURN_1_MARKER_ALPHA');
    const narrowMarker2 = extractMarkerLines(snapAfterNarrow, 'TURN_2_MARKER_BRAVO');
    const narrowMarker3 = extractMarkerLines(snapAfterNarrow, 'TURN_3_MARKER_CHARLIE');
    const narrowMarkerWide = extractMarkerLines(snapAfterNarrow, 'POST_RESIZE_WIDE_DELTA');
    ...
    await testCase.pushSendMessageResponse([
      { kind: 'event', data: { kind: 'AssistantResponseEvent', data: { content: 'POST_RESIZE_NARROW_ECHO' } } },
    ]);
    await testCase.pushSendMessageResponse(null);

    await testCase.sendKeys('narrow');
    await testCase.sleepMs(100);
    await testCase.pressEnter();
    await testCase.waitForText('POST_RESIZE_NARROW_ECHO', 15000);
    await testCase.waitForIdle(10000);
    ...
    expect(snapFinal.some(l => l.includes('POST_RESIZE_NARROW_ECHO'))).toBe(true);
    ...
    expect(extractMarkerLines(snapFinal, 'TURN_3_MARKER_CHARLIE').length).toBe(1);
    expect(extractMarkerLines(snapFinal, 'POST_RESIZE_WIDE_DELTA').length).toBe(1);
    expect(extractMarkerLines(snapFinal, 'POST_RESIZE_NARROW_ECHO').length).toBe(1);
  }, 90000);
});
