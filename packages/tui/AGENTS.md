...
- **Prerequisites:** E2E tests require both `target/debug/chat_cli` (Rust binary) and `dist/tui.js` (TUI bundle). `bun run test:e2e` builds both automatically; `--skip-rust-build` skips only the Rust build. Running individual files with `bun test` also auto-rebuilds the TUI bundle via a preload script.
